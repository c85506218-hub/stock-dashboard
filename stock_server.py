...
WATCHLIST = [
    ...
    ("2330.TW", "台積電 2330",       "台股個股", "NTD"),
    ("2317.TW", "鴻海 2317",         "台股個股", "NTD"),
    ("2454.TW", "聯發科 2454",       "台股個股", "NTD"),
...
SECTIONS_ORDER = ["台股大盤", "台股個股", "美股大盤", "美股個股"]
...
.ticker-item {
  ...
  padding: 0 20px; font-size: .76rem;
  font-variant-numeric: tabular-nums; letter-spacing: .01em;
}
.ticker-name { color: #64748b; }
.ticker-price { color: #94a3b8; }
.ticker-sep { color: #1e2d45; padding: 0 6px; font-size: .8rem; }
.ticker-news-item { padding: 0 20px; font-size: .72rem; color: #475569; }
.ticker-news-item a { color: #60a5fa; text-decoration: none; }
.ticker-news-item a:hover { color: #93c5fd; }
.ticker-alert { color: #fbbf24; font-size: .7rem; margin-right: 3px; }
...
<div class="ticker-bar">
  <div class="ticker-track" id="ticker-track">
    <span class="ticker-item"><span class="ticker-name">載入中…</span></span>
  </div>
</div>
...
function renderTicker(data){
  ...
  const cu     = q => (CUR[q.currency]||"");

  // 股票項目
  const stockItems = quotes.filter(q=>!q.error).map(q => {
    const pct  = q.change_pct;
    const up   = pct > 0;
    const big  = Math.abs(pct) >= 2;           // 重大漲跌
    const alert = big ? (up ? "🔥" : "⚠️") : "";
    const color = up ? "#4ade80" : pct < 0 ? "#f87171" : "#94a3b8";
    const sign  = pct >= 0 ? "+" : "";
    return `<span class="ticker-item">
      ${alert ? `<span class="ticker-alert">${alert}</span>` : ""}
      <span class="ticker-name">${q.name}</span>
      <span class="ticker-price">${cu(q)}${fmt(q.price)}</span>
      <span style="color:${color};font-weight:600">${arr(pct)} ${sign}${fmt(pct)}%</span>
    </span><span class="ticker-sep">｜</span>`;
  }).join("");

  // 最新重大新聞（所有股票最新一則）
  const seen = new Set();
  const newsItems = quotes.flatMap(q =>
    (news[q.ticker]||[]).slice(0,1)
  ).filter(a => {
    if(!a || seen.has(a.url)) return false;
    seen.add(a.url); return true;
  }).map(a =>
    `<span class="ticker-news-item">📰 <a href="${a.url}" target="_blank" rel="noopener">${a.title}</a></span>
     <span class="ticker-sep">｜</span>`
  ).join("");

  // 內容重複兩份讓動畫無縫銜接
  const content = stockItems + newsItems;
  const track = document.getElementById("ticker-track");
  track.innerHTML = content + content;   // 複製兩份：後半銜接前半

  // 依內容長度動態調整速度（避免太快或太慢）
  const itemCount = quotes.filter(q=>!q.error).length + Object.keys(news).length;
  const duration  = Math.max(40, itemCount * 5);
